####################################################################################################################
# Python Qt5 Testbed Tester Show BGP Seed Analysis File Builder
# MODULE:  ShowBgpSeedAnalysisFileBuilder
...
#------------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------------
# Show Bgp Seed Analysis File Builder
#-----------------------------------------------------------------------
class ShowBgpSeedAnalysisFileBuilder:
  "Show Bgp Seed Analysis File Builder"
  #----------------------------------------------------------------------------------------------------------------
  def __init__( self, parent = None ):
    self.name = "Show Bgp Seed Analysis File Builder"
    self.parent = parent
  ...
  def execute( self ):
    ...
    if self.cmd_list[SeedCommandDictionaryProcessor.verbose] == 'Yes':
      self.message_str = \
        "Build Show Bgp Seed File command started at: {}\n".\
    ...
    try:
      ...
      raise Exception( "{}: ShowBgpSeedAnalysisFileBuilder file I/O error with \"{}\". "
                       "Build Show Bgp Seed File Command terminated. ".
                       format( self.name,
    ...
    if self.device == "":
      raise CriticalFailure( "ShowBgpSeedAnalysisFileBuilder: Invalid seed file!" )
    try:
        ...
        for self.bgp_data in self.bgp_data_fd:
          ...
          if self.start_analysis_flag and self.device == "juniper":
            ...
            break
               ...
               self.bgp_data.find( ")-> show bgp" ) != -1:
            self.start_analysis_flag = True
            continue
          else:
            continue
    except Exception as e:
      raise Exception( "ShowBgpSeedAnalysisFileBuilder: {}".format(e) )
    #--------------------------------------------------------------------------------------------------------------
    if self.cmd_list[SeedCommandDictionaryProcessor.verbose] == 'Yes':
      self.message_str = \
  ...
  def build_cisco_analysis_seed_file( self ):
    ...
    for self.bgp_data in self.bgp_data_fd:
      ...
      if self.bgp_data.startswith( "---" ):
        continue
      if not self.bgp_data.startswith( "Address Family:" ):
        continue
      else:
        self.address_family_flag = True
        ...
        for self.bgp_data in self.bgp_data_fd:
          ...
          if self.bgp_data.startswith( "Address Family:" ):
            self.bgp_data_fd.seek( self.index -len( self.bgp_data ) )
            self.empty_address_family_flag = True
            break
          if self.address_family_flag == False:
            break
          if self.bgp_data.startswith( "\n" ):  # NOTE, this must be before bgp_data_list or it traps!!!!
            continue
          if self.bgp_data.startswith( "-----" ):
            continue
          if self.bgp_data.startswith( "Status codes" ):
            continue
          if self.bgp_data.startswith( "Origin codes" ):
            continue
          try:
            if self.bgp_data_list[0].startswith( "Network" ) or \
               self.bgp_data_list[2].startswith( "internal," ):
              continue
          except:
            pass
