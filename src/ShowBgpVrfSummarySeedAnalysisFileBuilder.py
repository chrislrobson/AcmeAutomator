####################################################################################################################
# Python Qt5 Testbed Tester Show BGP VRF Summary Seed Analysis File Builder
# MODULE:  ShowBgpVrfSummarySeedAnalysisFileBuilder
...
#------------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------------
# Show Bgp Vrf Summary Seed Analysis File Builder
#-----------------------------------------------------------------------
class ShowBgpVrfSummarySeedAnalysisFileBuilder:
  "Show Bgp Vrf Summary Seed Analysis File Builder"
  #----------------------------------------------------------------------------------------------------------------
  def __init__( self, parent = None ):
    self.name = "Show Bgp Vrf Summary Seed Analysis File Builder"
    self.parent = parent
  ...
  def execute( self ):
    ...
    if self.cmd_list[SeedCommandDictionaryProcessor.verbose] == 'Yes':
      self.message_str = \
        "Build Show Bgp Vrf Summary Seed File command started at: {}\n".\
    ...
    try:
      ...
      raise Exception( "{}: ShowBgpVrfSummarySeedAnalysisFileBuilder file I/O error with \"{}\". "
                       "Build Show Bgp Vrf Summary Seed File Command terminated. ".
                       format( self.name,
    ...
    if self.device == "":
      raise CriticalFailure( "ShowBgpVrfSummarySeedAnalysisFileBuilder: Invalid seed file!" )
    try:
      ...
      raise Exception( "ShowBgpVrfSummarySeedAnalysisFileBuilder: {}".format(e) )
    return()
  #----------------------------------------------------------------------------------------------------------------
      ...
      if self.bgp_data.startswith( "\n" ):
        ...
        if self.bgp_data.startswith( "BGP VRF" ):
          try:
            ...
            self.bgp_vrf_name = ""
          try:
            self.bgp_vrf_state = self.bgp_data_list[4]
          except:
            self.bgp_vrf_state = ""
          continue
        if self.bgp_data.startswith( "BGP Route Distinguisher" ):
          try:
            self.bgp_route_distinguisher = self.bgp_data_list[3].replace( ":", "~" )
          except:
            self.bgp_route_distinguisher = ""
          continue
        if self.bgp_data.startswith( "VRF ID" ):
          try:
            self.bgp_vrf_id = self.bgp_data_list[2]
          except:
            self.bgp_vrf_id = ""
          continue
