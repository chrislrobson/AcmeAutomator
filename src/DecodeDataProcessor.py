...
"""
CLASS: Interface
DESCRIPTION: Processes either "Interface" or "Interface Type"
INPUT:
OUTPUT:
"""
class Interface:
    ...
    """"""
    def execute(self, dictionary = None, descriptor = None, data = None, decode = None):
        if data.startswith("Interface Type"):
            self.interface_type(dictionary = dictionary, descriptor = descriptor, data = data, decode = decode)
        else:
            self.interface(dictionary = dictionary, descriptor = descriptor, data = data, decode = decode)
        return()
    """
  Processes the 'show interface' 'Interface [device name]' string
  """
    def interface(self, dictionary = None, descriptor = None, data = None, decode = None):
        self.dataFD = descriptor
        self.dictionary = dictionary
        self.data = data
        self.data_list = self.data.split()
        self.interface_received = self.data_list[1]
        self.decoder_instance.decoder_class_name_key = "physical interface {}".format(self.interface_received)
        try:
            self.value = "\"interface\":\"{}\"," \
                         "\"administrative state\":\"{}\"," \
                         "\"line protocol\":\"{}\"".format(self.interface_received,
                                                           self.data.split("is")[1].split(",")[0].lstrip(),
                                                           self.data.split("is")[2].split("\n")[0].lstrip().strip()
                                                           )
        except:
            """
            Fill through because line protocol isnt provided but we need for the analysis engine so
            force an entry and just use the admin state's status
            """
            try:
                self.value = "\"interface\":\"{}\"," \
                             "\"administrative state\":\"{}\"," \
                             "\"line protocol\":\"{}\"".format(self.interface_received,
                                                               self.data.split("is")[1].lstrip(),
                                                               self.data.split("is")[1].lstrip()
                                                               )
            except Exception as error:
                self.message = "{{{}{}: {}{}}}".format(Globals.RED_MESSAGE, self.name, error, Globals.SPAN_END_MESSAGE)
                ...
                raise Exception( error )
        self.decoder_instance.decoded_data[self.decoder_instance.decoder_class_name_key + " " +
                                           self.interface_received] = "{{{}}}".format(self.value)
        return()
    """
  Processes the 'show interface' 'Interface Type [variable data]' string
  """
    def interface_type(self, dictionary = None, descriptor = None, data = None, decode = None):
        self.dataFD = descriptor
        self.dictionary = dictionary
        self.data = data
        self.data_list = self.data.split()
        self.offset = 2
        self.key = " ".join(self.data_list[:self.offset])
        self.value = " ".join(self.data_list[self.offset:])
        self.decodedata = "\"{}\":\"{}\"".format(self.key, self.value)
        self.decoder_instance.decoded_data[self.decoder_instance.decoder_class_name_key +
                                           self.decoder_instance.decoder_class_name_key_added] = \
                                           "{{{}}}".format( self.decodedata )
        return()
"""
CLASS: KeyValueSplit
DESCRIPTION:
...
class KeyValueSplit:
    ...
    def execute(self, dictionary = None, descriptor = None, data = None, decode = None):
        ...
        self.decodedata = ""
        try:
            for self.index in range(0, len(self.data_list), 2):
                self.decodedata += "\"{}\":\"{}\",".format(self.data_list[self.index], self.data_list[self.index + 1].replace(":", "."))
        except Exception as error:
            self.message = "{{{}{}: {}{}}}".format(Globals.RED_MESSAGE, self.name, error, Globals.SPAN_END_MESSAGE)
            self.dictionary['loggerwidget'].emit(self.message)
            raise Exception
        self.decodedata = self.decodedata[:-1]
        self.decoder_instance.decoded_data[self.decoder_instance.decoder_class_name_key +
                                           self.decoder_instance.decoder_class_name_key_added] = \
                                           "{{{}}}".format( self.decodedata )
        return()
...
class Inet:
    ...
    def execute(self, dictionary=None, descriptor=None, data=None, decode=None):
        ...
        self.decoder_instance.decoder_class_name_key_added = self.data_list[0]
        if self.data_list[0].startswith("inet6"):
            self.inet6(dictionary=dictionary, descriptor=descriptor, data=data, inetv="inet6")
        elif self.data_list[0].startswith("inet"):
            self.inet4(dictionary=dictionary, descriptor=descriptor, data=data, inetv="inet4")
        else:
            self.message = "{{{}{}: {}{}}}".format(Globals.RED_MESSAGE, self.name, "Invalid inet type", Globals.SPAN_END_MESSAGE)
            self.dictionary['loggerwidget'].emit(self.message)
            raise
        return ()
    """"""
    def inet4(self, dictionary = None, descriptor = None, data = None, inetv = None):
        self.inetv = inetv
        self.dataFD = descriptor
        self.dictionary = dictionary
        self.data = data
        ...
        try:
            for self.index in range(0, len(self.data_list), 2):
                try:
                    ...
                    self.decodedata += "\"{}\":\"\",".format(self.data_list[self.index])
        except Exception as error:
            self.message = "{{{}{}: {}{}}}".format(Globals.RED_MESSAGE, self.name, error, Globals.SPAN_END_MESSAGE)
            self.dictionary['loggerwidget'].emit(self.message)
            raise Exception
        ...
        return()
    """"""
    def inet6(self, dictionary = None, descriptor = None, data = None, inetv = None):
        self.inetv = inetv
        self.dataFD = descriptor
        self.dictionary = dictionary
        self.data = data
        self.data_list = self.data.split()
        try:
            self.hwaddress = self.data_list[1].replace(":", ".")
        except Exception as error:
            self.message = "{{{}{}: {}{}}}".format(Globals.RED_MESSAGE, self.name, error, Globals.SPAN_END_MESSAGE)
            self.dictionary['loggerwidget'].emit(self.message)
            raise Exception(error)
        self.value = "\"{}\":\"{}\"".format(self.inetv, self.hwaddress)
        self.decoder_instance.decoded_data[self.decoder_instance.decoder_class_name_key +
                                           self.decoder_instance.decoder_class_name_key_added] = \
                                           "{{{}}}".format(self.value)
        return()
"""
CLASS: HWaddr
DESCRIPTION: Process FRRouting HWaddr show interface hardware address string
INPUT:
OUTPUT:
"""
class HWaddr:
    ...
    def execute(self, dictionary = None, descriptor = None, data = None, decode = None):
        ...
        try:
            ...
            self.dictionary['loggerwidget'].emit(self.message)
            raise Exception( error )
        self.value = "\"hwaddr\":\"{}\"".format(self.hwaddress)
        self.decoder_instance.decoded_data[self.decoder_instance.decoder_class_name_key +
                                           self.decoder_instance.decoder_class_name_key_added] = \
                                           "{{{}}}".format( self.value )
        return()
