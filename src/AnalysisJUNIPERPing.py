####################################################################################################################
# Python Qt5 Testbed Tester Analysis JUNIPER Ping
# MODULE:  AnalysisJUNIPERPing
...
#------------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------------
# JUNIPER Ping analysis processor
#-----------------------------------------------------------------------
class JUNIPERPing:
  "JUNIPER Ping"
  #----------------------------------------------------------------------------------------------------------------
  def __init__( self, parent = None ):
    self.name = "JUNIPER Ping Analysis"
    self.parent = parent
  ...
  def execute( self ):
    ...
    try:
      with open( self.cmd_list[SeedCommandDictionaryProcessor.commandpath] +
        ...
        self.message_str = "Processing JUNIPER Ping Analysis file: {}.".\
                    ...
                    if self.targetip in self.report_data:
                      while( not self.report_data.startswith( "DUT=======" ) ):
                        try:
                          ...
                          sys.exc_info()
                          break
                        if "packets transmitted" in self.report_data and \
                           "packets received" in self.report_data and "packet loss":
                          self.successfull_pings = self.report_data.split()[0]
                          self.failed_pings = self.report_data.split()[3]
                          break
                else:
