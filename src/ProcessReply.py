...
from Exceptions import MyExceptions,SwitchProcessorBusy
...
class SendYes:
    ...
    def execute(self, dictionary = None, descriptor = None, data = None, decode = None):
        ...
        try:
            ...
            self.reply_action = self.dictionary['processreply'].find_prompt(self.received_data, self.dictionary['processreply'].prompt_automation)
    ...
    def execute(self, dictionary = None, descriptor = None, data = None, decode = None):
        ...
        try:
            ...
            if self.dictionary['verbose']:
                ...
                self.dictionary['loggerwidget'].emit(self.message)
            self.reply_action = self.dictionary['processreply'].scan_reply(self.received_data, self.dictionary['processreply'].prompt_automation)
            for self.action_type in self.reply_action:
                if not "Continue" in self.action_type:
                    self.reply = self.action_type[1]
                    break
                ...
                """
                No WAS set up to catch any following commits the engineer has in the seed file.  Answer "no" so the
                original configuration is not be whipped out.  AKA, another crappy F'ed up Cisco code logic.
    ...
    def execute(self, dictionary = None, descriptor = None, data = None, decode = None):
        ...
        try:
            ...
            for self.action_type in self.reply_action:
                ...
                """
                No set up to catch any following commits the engineer has in the seed file.  Answer "no" so the
                original configuration is not be whipped out.  AKA, another crappy Cisco code logic.
                After answering "no", program resets the "Do you wish to proceed to the default "yes"
                """
                if "Continue" in self.action_type:
...
class ReturnRequest:
    ...
    def __init__(self, parent=None):
        ...
        self.ssh_handle = parent.ssh_handle
        self.session_state = {
            "starttelnet":"SendCarriageReturn",
            "stoptelnet":"TerminateTelnet",
        }
    """"""
    def execute(self, dictionary = None, descriptor = None, data = None, decode = None):
        self.dictionary = dictionary
        self.received_data = data
        try:
            self.results = CallClass(self).call_class(module_name = "ProcessReply", class_name = self.session_state[self.dictionary['session']],
                                                      method_name = "execute", dictionary = self.dictionary, data = self.received_data)
        except Exception as error:
            self.message = "{{{}{}: {}{}}}".format(Globals.RED_MESSAGE, self.name, error, Globals.SPAN_END_MESSAGE)
            self.dictionary['loggerwidget'].emit(self.message)
...
class TelnetPrompt:
    ...
    def execute(self, dictionary = None, descriptor = None, data = None, decode = None):
        ...
        return ()
"""
CLASS: TerminateTelnet
DESCRIPTION: Send Control-C Right-Bracket and Ctrol-C to terminate current telnet session
INPUT: 
OUTPUT: Errors cause exceptions
"""
class TerminateTelnet:
    ...
    def execute(self, dictionary = None, descriptor = None, data = None, decode = None):
        ...
        self.received_data = data
        try:
            self.received_data = SendDataThroughSSHChannel(self).send_data_through_ssh_channel(dictionary = self.dictionary, data = "\x1D\n")
            if self.dictionary['verbose']:
                self.message = "Control-] sent to DUT.\n{}".format(self.received_data)
                self.dictionary['loggerwidget'].emit(self.message)
            self.reply_action = self.dictionary['processreply'].scan_reply(self.received_data, self.dictionary['processreply'].prompt_automation)
            for self.action_type in self.reply_action:
                if not "Continue" in self.action_type:
...
"""
CLASS: SendQuit
DESCRIPTION: Send quit to terminate the ssh session
INPUT: Received data
OUTPUT: Errors cause exceptions
"""
class SendQuit:
    ...
    def execute(self, dictionary = None, descriptor = None, data = None, decode = None):
        ...
        self.received_data = data
        try:
            self.received_data = SendDataThroughSSHChannel(self).send_data_through_ssh_channel(dictionary = self.dictionary, data = "quit\n")
            if self.dictionary['verbose']:
                self.message = "Quit sent to DUT.\n{}".format(self.received_data)
                self.dictionary['loggerwidget'].emit(self.message)
            self.reply_action = self.dictionary['processreply'].scan_reply(self.received_data, self.dictionary['processreply'].prompt_automation)
            for self.action_type in self.reply_action:
                if not "Continue" in self.action_type:
...
"""
CLASS: SendExit
DESCRIPTION: Send exit to terminate SSH session
INPUT: Received data
OUTPUT: Errors cause exceptions
"""
class SendExit:
    ...
    def execute(self, dictionary = None, descriptor = None, data = None, decode = None):
        ...
        self.received_data = data
        try:
            self.received_data = SendDataThroughSSHChannel(self).send_data_through_ssh_channel(dictionary = self.dictionary, data = "exit\n")
            if self.dictionary['verbose']:
                self.message = "Exit sent to DUT.\n{}".format(self.received_data)
                self.dictionary['loggerwidget'].emit(self.message)
            self.reply_action = self.dictionary['processreply'].scan_reply(self.received_data, self.dictionary['processreply'].prompt_automation)
            for self.action_type in self.reply_action:
                if not "Continue" in self.action_type:
...
class SendUsername:
    ...
    def execute(self, dictionary = None, descriptor = None, data = None, decode = None):
        self.dictionary = dictionary
        self.user = self.dictionary['user']
        self.data = data
        try:
            self.received_data = SendDataThroughSSHChannel(self).send_data_through_ssh_channel(dictionary = self.dictionary, data = self.user)
            if self.dictionary['verbose']:
...
class SendAbort:
    ...
    def execute(self, dictionary = None, descriptor = None, data = None, decode = None):
        ...
        try:
            ...
            if self.dictionary['verbose']:
                self.message = "{{{}Abort sent to DUT{}}}".format(Globals.BLACK_MESSAGE, Globals.SPAN_END_MESSAGE)
                self.dictionary['loggerwidget'].emit(self.message)
            self.reply_action = self.dictionary['processreply'].find_prompt(self.received_data, self.dictionary['processreply'].prompt_automation)
            for self.action_type in self.reply_action:
                if not "Continue" in self.action_type:
            ...
            self.dictionary['loggerwidget'].emit(self.message)
            raise Exception
        self.message = "{{{}An abort was sent to DUT, terminating all processes!{}}}".format(Globals.RED_MESSAGE, Globals.SPAN_END_MESSAGE)
        self.dictionary['loggerwidget'].emit(self.message)
        raise Exception
"""
CLASS: Continue
