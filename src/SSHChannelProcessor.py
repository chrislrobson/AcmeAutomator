...
class SendDataThroughSSHChannel(SSHChannel):
  ...
  def send_data_through_ssh_channel(self, dictionary = None, data = "", delay = .5, read_timeout = 90):
    ...
    try:
      ...
      # todo-debug
      self.ssh_connection.sendall(self.normalize_transmitting_data(self.data))
      """
      ...
      router and being read by Paramiko has completed the transfer.  By putting a half-second delay (or more) this
    ...
    for justincaseloopcnt in range(512):
      ...
      if self.ssh_connection.recv_ready():
        ...
        try:
          ...
          if self.last_word:
            self.reply_action = self.cmd_dict['processreply'].find_prompt(self.last_word, self.cmd_dict['processreply'].prompt_automaton)
            for self.action_type in self.reply_action:
              """
              More Cisco crap, when sending scp data, Cisco, if command is abort, split the data into two transmission
              which of course hoses any logic scanning for command terminations sequences, ass-hole!
              """
              if "ReceiveMoreData" in self.action_type:
                self.reply_action = []
                break
        ...
        self.elapse_time = time.time() + read_timeout
      # notes  ------!!!!!!!!!!!!!!!!!
      # notes  This is here for a reminder and historical.  This is how it should be however
      # notes  because of Cisco lame as code it cant be done ergo why the above HORRIBLE code
      # notes  is done, Maybe I can find a better way later, I HATE CISCO They write HORRIBLE OS CODE!!!!
      # notes  if self.ssh_connection.recv_ready():
      # notes    try:
      # notes      self.output += self.ssh_connection.recv( self.data_buffer_size ).decode('utf-8', 'ignore')
      # notes    except Exception as error:
      # notes      pass
      # notes    try:
      # notes      self.output_normalized = NormalizeData( self.parent ).normalize_data( self.output )
      # notes      self.prompt_str = ssh_connection_dict['processreply'].find_prompt( self.output_normalized,
      # notes                                                        ssh_connection_dict['processreply'].prompt_automaton
      # notes                                                      )
      # notes     if self.prompt_str:
      # notes       break
      # notes   except:
      # notes     pass
      # notes   self.elapse_time = time.time( ) + read_timeout
      else:
        time.sleep(self.delay * 1)
        # todo-debug
  ...
  """"""
  @staticmethod
  def normalize_transmitting_data(data):
    try:
      data = data.rstrip("\n")
    except Exception as error:
