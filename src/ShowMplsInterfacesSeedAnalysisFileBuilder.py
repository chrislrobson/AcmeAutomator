####################################################################################################################
# Python Qt5 Testbed Tester Show MPLS Interfaces Seed Analysis File Builder
# MODULE:  ShowMplsInterfacesSeedAnalysisFileBuilder
...
#------------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------------
# Show Mpls Interfaces Seed Analysis File Builder
#-----------------------------------------------------------------------
class ShowMplsInterfacesSeedAnalysisFileBuilder:
  "Show Mpls Interfaces Seed Analysis File Builder"
  #----------------------------------------------------------------------------------------------------------------
  def __init__( self, parent = None ):
    self.name = "Show Mpls Interfaces Seed Analysis File Builder"
    self.parent = parent
  ...
  def execute( self ):
    ...
    if self.cmd_list[SeedCommandDictionaryProcessor.verbose] == 'Yes':
      self.message_str = \
        "Build Show Mpls Interfaces Seed File command started at: {}\n".\
    ...
    try:
      ...
      raise Exception( "{}: ShowMplsInterfacesSeedAnalysisFileBuilder file I/O error with \"{}\". "
                       "Build Show Mpls Interfaces Seed File Command terminated. ".
                       format( self.name,
    ...
    if self.device == "":
      raise CriticalFailure( "ShowMplsInterfacesSeedAnalysisFileBuilder: Invalid seed file!" )
    try:
        ...
        for self.mpls_data in self.mpls_data_fd:
          if self.start_analysis_flag and self.device == "juniper":
            ...
            break
               ...
               self.mpls_data.find( ")-> show mpls interfaces" ) != -1:
            self.start_analysis_flag = True
            continue
          else:
            continue
    except Exception as e:
      raise Exception( "ShowMplsInterfacesSeedAnalysisFileBuilder: {}".format(e) )
    return()
  #----------------------------------------------------------------------------------------------------------------
  ...
  def build_cisco_analysis_seed_file( self ):
    ...
    for self.mpls_data in self.mpls_data_fd:
      ...
      if not self.title.startswith( "InterfaceLDPTunnelStaticEnabled" ):
        continue
      else:
        for self.mpls_data in self.mpls_data_fd:
          if self.mpls_data.startswith( "\n" ):
          ...
          if self.mpls_data.startswith( "-------------------------- --------" ):
            continue
          try:
            self.mpls_interfaces_received = self.mpls_data_list[0]
          except:
            self.mpls_interfaces_received = ""
          try:
            self.mpls_ldp_received = self.mpls_data_list[1]
          except:
            self.mpls_ldp_received = ""
          try:
            self.mpls_tunnel_received = self.mpls_data_list[2]
          except:
            self.mpls_tunnel_received = ""
          try:
            self.mpls_static_received = self.mpls_data_list[3]
          except:
            self.mpls_static_received = ""
          try:
            self.mpls_enabled_received = self.mpls_data_list[4]
          except:
            self.mpls_enabled_received = ""
          #-----------------------------------------------------------------------------------------
