####################################################################################################################
# Python Qt5 Testbed Tester Linux IP Route Show
# MODULE:  LinuxIPRouteShow
...
# linuxiprouteshow
#-----------------------------------------------------------------------
class linuxiprouteshow:
  "default"
  #----------------------------------------------------------------------------------------------------------------
  def __init__( self, parent = None ):
    self.name = "Linux Ip Route Show"
    self.parent = parent
  ...
  def execute( self, seed_file, data_file ):
    if self.ggparent.testbed_tester.verbose:
      self.ggparent.logger_message_signal.emit( "Linux route table processing is running" )
    self.results = ""
    #----------------------------------------------------------------------------------------------------------------
    try:
      ...
      self.message_str = Globals.RED_MESSAGE + \
                         "LINUXIPROUTESHOW: data file I/O error with \"{}\".".format( seed_file ) + \
                         Globals.SPAN_END_MESSAGE
      self.ggparent.logger_message_signal.emit(self.message_str)
      self.message_str = Globals.RED_MESSAGE + \
                         "LINUXIPROUTESHOW: analysis terminated." + \
                         Globals.SPAN_END_MESSAGE
      self.ggparent.logger_message_signal.emit(self.message_str)
          ...
          if self.seed_data.startswith( "#" ) or \
            ...
            for self.line in self.dataFD:
              if self.line.find( self.keyword ) != -1:
                self.found = True
                if self.verbose == "Yes":
                  self.message_str = "FOUND {} via {}". \
                    format(self.line.split("\n")[0].split()[0],
                           self.line.split("\n")[0].split()[1])
                  self.ggparent.processor_message_signal.emit(self.message_str)
                  self.results += "FOUND {} via {}\n". \
                                  format(self.line.split("\n")[0].split()[0],
                                 self.line.split("\n")[0].split()[1])
                break
            if not self.found:
              self.message_str = Globals.RED_BOLD_ONLY_MESSAGE + \
                                 "NOT FOUND: " + \
                                 Globals.SPAN_END_MESSAGE
              self.message_str += "{} route". \
                                 format( self.keyword )
              self.ggparent.processor_message_signal.emit( self.message_str )
              self.results += "NOT FOUND: {} route\n".format( self.keyword )
            continue
    except:
      self.message_str = Globals.RED_MESSAGE + \
                         "LINUXIPROUTESHOW: File I/O error with \"{}\".". \
                           format( data_file + self.filename_time_extension ) + \
                         Globals.SPAN_END_MESSAGE
      self.ggparent.logger_message_signal.emit(self.message_str)
    self.message_str =  Globals.BLUE_FONT1O_BOLD_MESSAGE + \
                         "Route Analysis Report Completed." + \
                         Globals.SPAN_END_MESSAGE
    self.ggparent.processor_message_signal.emit(self.message_str)
    return( self.results )
#####################################################################################################################