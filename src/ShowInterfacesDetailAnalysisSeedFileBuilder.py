...
from BuildAnalysisSeedFile import BuildAnalysisSeedFile
"""
...
class ShowInterfacesDetailAnalysisSeedFileBuilder:
        ...
        try:
            with open( self.data_filename, "r" ) as self.detail_data_fd:
                ...
                for self.detail_data in self.detail_data_fd:
                    ...
                    """
                    if self.detail_data.startswith( "DUT(" ) and self.detail_data.find( ")-> show interface" ) != -1:
                    ...
                    # fixme fixme  THE FOLLOWING CODE BLOCK NEED TO BE RECODE USING IMPORTLIB OOP call!!!!!
                    """
                    ...
                    Cisco/FRRouting data string to match: ", line protocol is" 
                    """
                    if self.start_analysis_flag and self.detail_data.startswith( "Physical interface:" ) or self.detail_data.find( ", line protocol is" ) != -1:
                        ...
                        if self.dictionary["device"] == "juniper":
                            try:
                                self.reportFD = self.build_multi_router_analysis_seed_file(self.analysis_fd, self.dictionary)
                            except Exception as error:
                                if error.args:
                                    self.message = "{{{}{}: {}{}}}".format(Globals.RED_MESSAGE, self.name, error, Globals.SPAN_END_MESSAGE)
                                    self.dictionary['loggerwidget'].emit(self.message)
                        ...
                        if self.start_analysis_flag and self.dictionary["device"] == "cisco":
                            try:
                                ...
                                raise
                            break
                        if self.start_analysis_flag and self.dictionary["device"] == "frrouting":
                            try:
                                self.reportFD = self.build_multi_router_analysis_seed_file(self.analysis_fd, self.dictionary)
                            except Exception as error:
                                if error.args:
                                ...
                                raise
                            break
                            # fixme fixme  THE ABOVE CODE BLOCK NEED TO BE RECODE USING IMPORTLIB OOP call!!!!!
    ...
    def build_cisco_analysis_seed_file(self, analysis_fd, dictionary):
                ...
                if not self.interface_found_flag:
                    ...
                    for self.detail_data in self.detail_data_fd:
                        ...
                        if self.detail_data.startswith( "\n" ):
                            ...
                            """
                            BuildAnalysisSeedFile(self).build_analysis_seed_file_entry(dictionary = self.dictionary, decoder_instance = self.decoder_instance)
        ...
        """
        try:
            WriteAnalysisSeedFile(self).write_analysis_seed_file(table=self.show_interfaces_detail_table, analysis_fd=self.analysis_fd, dictionary=self.dictionary)
        except:
            raise
        return ()
    """
    ...
    OUTPUT: Analysis seed file data
    """
    def build_multi_router_analysis_seed_file(self, analysis_fd, dictionary):
        self.dictionary = dictionary
        self.analysis_fd = analysis_fd
        self.current_interface = ""
        self.show_interfaces_detail_table = []
        self.intf_idx = {"juniper":2,"cisco":1,"frrouting":1,}
        """
        ...
        for self.detail_data in self.detail_data_fd:
            ...
            try:
                ...
                self.message = "{{{}{}: Juniper/FRRouting interface processing error: {}{}}}".format(Globals.RED_MESSAGE, self.name, error, Globals.SPAN_END_MESSAGE)
                self.dictionary['loggerwidget'].emit(self.message)
                raise
            if self.detail_data.startswith("Physical interface:") or self.detail_data.startswith("Interface "):
                for self.intf in self.interfaces_to_match.split():
                    self.current_interface = self.detail_data_list[self.intf_idx[self.dictionary["device"]]]
                    if self.detail_data_list[self.intf_idx[self.dictionary["device"]]].startswith( self.intf ):
                ...
                if not self.start_processing_details_flag:
                    ...
                    for self.detail_data in self.detail_data_fd:
                        """
                        Stop processing current "Interface" block of data ends
                        """
                        try:
                            if self.detail_data.startswith( "Physical interface:" ) and \
                                    self.current_interface != self.detail_data.split()[self.intf_idx[self.dictionary["device"]]] or \
                                    self.detail_data.startswith( "Interface " ) and self.current_interface != self.detail_data.split()[self.intf_idx[self.dictionary["device"]]]:
                                BuildAnalysisSeedFile(self).build_analysis_seed_file_entry(dictionary = self.dictionary, decoder_instance = self.decoder_instance)
                                self.decoder_instance.decoder_class = ""
                                self.decoder_instance.decoder_class_name = ""
                                self.decoder_instance.decoder_class_name_key = ""
                        ...
                        if self.detail_data.startswith( "\n" ):
                            continue
                        try:
                            Decoder(self).decode(decoder_instance = self.decoder_instance,
                            ...
                            self.message = "{{{}{}({}): {} is an unknown data decoder type, therefore, requires addition to the " \
                                           "DecoderDataDictionary.py file.{}}}".format(Globals.RED_MESSAGE, self.name,
                                                                                       self.dictionary['device'],
                                                                                       self.detail_data.split()[0],
                                                                                       Globals.SPAN_END_MESSAGE)
                            self.dictionary['loggerwidget'].emit(self.message)
                            raise
                    else:
                        if self.decoder_instance.decoder_class:
                            BuildAnalysisSeedFile(self).build_analysis_seed_file_entry(dictionary=self.dictionary,
