...
class Command:
    ...
    def __init__( self, parent = None ):
        ...
        self.logger_message_signal = self.parent.dictionary['loggerwidget']
    """"""
        ...
        try:
            if self.dictionary['fileoutput']:
                ...
                try:
                    ...
                    self.message = "{{{}{}: {} with {}{}}}".format(Globals.RED_MESSAGE, self.name, error, self.dictionary['archivefilename'],
                ...
                for self.cmd_data in self.commandsFD:
                    if not SeedCommandlinePreprocessor(self).preprocessor(message_signal = self.dictionary['loggerwidget'], data = self.cmd_data):
                        ...
                        try:
                            ...
                            self.received_data = \
                                SendDataThroughSSHChannel(self).send_data_through_ssh_channel(dictionary = self.dictionary,
                        ...
                        The master seed file "fullscan" is used for commands like a configure>load command sequence which
                        could fail if the file is corrupted or not present in some way, therefore the recieved data needs a
                        full scan of the entire receive buffer which may contain error replies and process accorndingly.
                        """
                        if self.dictionary['fullscan']:
                            self.reply_action = \
                                self.dictionary['processreply'].find_prompt_full_scan(self.received_data,
                                                                                      self.dictionary['processreply'].prompt_automation)
                        else:
                            self.reply_action = self.dictionary['processreply'].find_prompt(self.received_data,
                                                                                            self.dictionary['processreply'].prompt_automation)
                        ...
                        if self.received_data_norm:
                            if self.archivefileFD != None:
                                try:
                                    try:
                                        ...
                                        self.hostname = ""
                                    self.archivefileFD.write("DUT( {}/{})-> {}\n".format(self.hostname, self.dictionary['ip'],
                                                                                         self.cmd_data.split("\n")[0]))
                                except Exception as error:
                                    self.message = "{{{}{}: {}{}}}".format(Globals.RED_MESSAGE, self.name, error, Globals.SPAN_END_MESSAGE)
                                    self.dictionary['loggerwidget'].emit(self.message)
                            ...
                            if self.dictionary['stdout']:
                                ...
                                try:
                                    self.report_widget, self.tab_index = \
                                        ManageTabWidgets().get_tab_list_widget(tab_widgets = self.dictionary['widget'],
                                                                               tab_id = self.dictionary['tab_id'],
                                                                               list_widget = self.dictionary['tab_id'])
                                except Exception as error:
                                    self.message = "{{{}{}: {}{}}}".format(Globals.RED_MESSAGE, self.name, error, Globals.SPAN_END_MESSAGE)
                                    self.dictionary['loggerwidget'].emit(self.message)
                                    self.archivefileFD.close()
    ...
    def preprocess_received_data(self, dictionary = None, data = None):
        self.data = data
        self.dictionary = dictionary
        # fixme NEED TO MAKE THIS A LIST LOOKUP TO HANDLE FOR TYPES THAT ARE LIKE CISCO or JUNIPER FOR THAT MATTER!!!!!!
        if self.dictionary['device'] == "cisco" or self.dictionary['device'] == "frrouting":
