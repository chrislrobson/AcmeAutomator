####################################################################################################################
# Python Qt5 Testbed Tester Show RSVP Neighbors Detail Seed Analysis File Builder
# MODULE:  ShowRsvpNeighborsDetailSeedAnalysisFileBuilder
...
#------------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------------
# Show Rsvp Neighbors Detail Seed Analysis File Builder
#-----------------------------------------------------------------------
class ShowRsvpNeighborsDetailSeedAnalysisFileBuilder:
  "Show Rsvp Neighbors Detail Seed Analysis File Builder"
  #----------------------------------------------------------------------------------------------------------------
  def __init__( self, parent = None ):
    self.name = "Show Rsvp Neighbors Detail Seed Analysis File Builder"
    self.parent = parent
  ...
  def execute( self ):
    ...
    if self.cmd_list[SeedCommandDictionaryProcessor.verbose] == 'Yes':
      self.message_str = \
        "Build Show Rsvp Neighbors Detail Seed File command started at: {}\n".\
    ...
    try:
      ...
      raise Exception( "{}: ShowRsvpNeighborsDetailSeedAnalysisFileBuilder file I/O error with \"{}\". "
                       "Build Show Rsvp Neighbors Detail Seed File Command terminated. ".
                       format( self.name,
    ...
    if self.device == "":
      raise CriticalFailure( "ShowRsvpNeighborsDetailSeedAnalysisFileBuilder: Invalid seed file!" )
    try:
      ...
      raise Exception( "ShowRsvpNeighborsDetailSeedAnalysisFileBuilder: {}".format(e) )
    return()
  #----------------------------------------------------------------------------------------------------------------
            ...
            if self.rsvp_data.startswith( "\n" ):
              ...
              if self.rsvp_data.startswith( "  Interface Neighbor" ):
                try:
                  ...
                  self.rsvp_interface_neighbor = ""
                for self.rsvp_data in self.rsvp_data_fd:
                  if self.rsvp_data.startswith( "Global Neighbor" ) or \
                      self.rsvp_data.startswith( "  Interface Neighbor" ):
                    self.rsvp_data_fd.seek( self.line_length)
                    break
                  self.line_length += len( self.rsvp_data )
                  if self.rsvp_data.startswith( "\n" ):
